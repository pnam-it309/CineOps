<template>
  <div class="movie-management w-100 h-100 bg-transparent overflow-hidden">
    <AdminTableLayout title="Quản lý Hóa Đơn" titleIcon="bi bi-receipt" :data="invoices" :loading="loading"
      :total="total" v-model:currentPage="listQuery.page" v-model:pageSize="listQuery.size" @reset-filter="handleReset">
      <template #filters>
        <div class="filter-item">
          <el-input v-model="listQuery.tuKhoa" placeholder="Mã đơn, Tên KH, SĐT..." :prefix-icon="Search" clearable
            @keyup.enter="handleFilter" @clear="handleFilter" style="width: 230px" />
        </div>

        <div class="filter-item">
          <el-select v-model="listQuery.trangThai" placeholder="Trạng thái" clearable @change="handleFilter"
            style="width: 140px">
        ...
        <div class="filter-item">
          <el-date-picker v-model="customDateRange" type="daterange" range-separator="-" start-placeholder="Từ ngày"
            end-placeholder="Đến ngày" format="DD/MM/YYYY" value-format="YYYY-MM-DD" @change="handleDateChange"
            style="width: 250px" />
        </div>

        <div class="filter-item">
          <el-select v-model="listQuery.kyThoiGian" placeholder="Kỳ thời gian" clearable @change="handlePeriodChange"
            style="width: 150px">
        ...
        <div class="filter-item">
          <el-select v-model="listQuery.phuongThucThanhToan" placeholder="Thanh toán" clearable @change="handleFilter"
            style="width: 130px">
      ...
      <template #content>
        <div class="h-100 overflow-hidden">
          <BaseTable v-loading="loading" :data="invoices" :columns="tableColumns" :total="total"
            v-model:current-page="listQuery.page" v-model:page-size="listQuery.size" :hide-pagination="true">
    ...
    <BaseModal v-model="modalVisible" :title="'Hồ sơ hóa đơn'" width="1100px" icon="bi bi-receipt-cutoff" isDetail
      onlyCancel>
        ...
        <div class="p-4 border-bottom bg-white d-flex align-items-center gap-4">
          ...
          <div class="flex-grow-1">
            ...
            <div class="d-flex align-items-center gap-3">
              <div class="small text-secondary"><i class="bi bi-calendar3 me-1"></i>Ngày lập: {{
                formatDate(detailInvoice.ngayTao) }}</div>
              <div class="small text-secondary"><i class="bi bi-wallet2 me-1"></i>{{ detailInvoice.phuongThucThanhToan
                === 0
                ? 'Tiền mặt' : 'Chuyển khoản' }}</div>
            </div>
          </div>
          <div class="text-end">
            ...
            <div class="col-lg-8">
                ...
                <table class="table table-hover m-0 w-100">
                  ...
                  <tbody>
                    <!-- <tr v-for="(item, idx) in invoiceDetails" :key="idx">
                      <td>
                        <div class="fw-bold">{{ item.loai === 1 ? (item.tenPhim || '') : (item.tenSanPham || '') }}</div>
                        <div v-if="item.loai === 1 && item.viTriGhe" class="small text-secondary">
                          Ghế: {{ item.viTriGhe }} 
                        </div>
                      </td>
              ...
              <div class="mt-4 p-4 border bg-light d-flex justify-content-end gap-5">
                <div class="text-end">
                  ...
                  <div class="fw-bold text-dark fs-5">{{formatCurrency(invoiceDetails.reduce((acc, i) => acc +
                    i.thanhTien,
                    0)) }}</div>
                </div>
            ...
            <div class="col-lg-4">
              ...
              <div class="p-3 border bg-light mt-3">
                ...
                <div class="d-flex justify-content-between">
                  ...
                  <span class="text-dark fw-bold">{{ formatCurrency((detailInvoice.tongTienThanhToan || 0) * 0.08)
                    }}</span>
                </div>
              </div>
            </div>
